<x-app-layout>
    ...
    {{-- Judul Halaman --}}
    <x-slot name="title">Data Kategori Bisnis</x-slot>

    {{-- Bagian Kategori Bisnis --}}
        ...
        <div class="bg-gray-50 rounded-lg border border-gray-300 shadow">
            <div class="p-2 space-y-2">
                ...
                <div x-data="{ openCreate: false }" class="flex flex-col lg:flex-row items-center justify-between gap-4">
                    ...
                    <div x-show="openCreate" class="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
                        ...
                        <div class="bg-white rounded-lg p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto">
                            <h2 class="text-lg font-semibold mb-4">Tambah Kategori Bisnis</h2>

                            <form method="POST" action="{{ route('dashboard.business-category.store') }}">
                                @csrf

                                <div class="grid grid-cols-1 gap-5">
                                    <x-forms.input name="name" label="Nama Kategori"></x-forms.input>
                                </div>
                    ...
                    <form method="GET" action="#" class="w-full flex justify-end gap-1" x-data="{ openFilter: '' }">
                        ...
                        <div class="w-full lg:w-80">
                            <x-forms.input type="text" name="search" placeholder="Cari kategori bisnis..."
                ...
                <div class="overflow-x-auto">
                    {{ $businessCategories->withQueryString()->links('pagination::custom') }}
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg border border-gray-300 shadow overflow-x-auto">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-300">
                    @forelse($businessCategories as $businessCategory)
                        <tr class="hover:bg-blue-50">
                            <td class="p-2 text-center border-r border-gray-300">
                                {{ ($businessCategories->currentPage() - 1) * $businessCategories->perPage() + $loop->iteration }}
                            </td>
                            <td class="p-2 border-r border-gray-300 whitespace-nowrap">{{ $businessCategory->name }}
                            </td>
                            <td class="p-2 text-center border-r border-gray-300 whitespace-nowrap">
                                {{ \Carbon\Carbon::parse($businessCategory->created_at)->format('d/m/Y H:i') }}
                            </td>
                            <td class="p-2 whitespace-nowrap">
                                <div class="flex justify-center items-center gap-2">
                                    <div x-data="{ openEdit: false }">
                                        ...
                                        {{-- Modal Edit Kategori Bisnis --}}
                                        <div x-show="openEdit"
                                            class="fixed inset-0 flex items-center justify-center bg-black/50 z-50">

                                            <div
                                                class="bg-white rounded-lg p-6 w-full max-w-lg max-h-[80vh] overflow-y-auto">
                                                <h2 class="text-lg font-semibold mb-4">Edit Kategori Bisnis</h2>

                                                <form method="POST"
                                                    action="{{ route('dashboard.business-category.update', $businessCategory->slug) }}">
                                                    @csrf
                                                    @method('PUT')

                                                    <div class="grid grid-cols-1 gap-5">
                                                        <x-forms.input name="name" label="Nama Kategori"
                                                            :value="old('name', $businessCategory->name)"></x-forms.input>
                                                    </div>
                                    ...
                                    <form
                                        action="{{ route('dashboard.business-category.destroy', $businessCategory->slug) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="p-4 text-center text-gray-500">Tidak ada data kategori bisnis
                            </td>
                        </tr>
                    @endforelse
                </tbody>
